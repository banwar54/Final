...
const Quiz = () => {
  ...
  const sampleQuestionData = {
    ...
    q4o1: "Charles Dickens",
    q4o2: "William Shakespeare",
    q4o3: "Jane Austen",
    q4o4: "Mark Twain",
    q4ans: "William Shakespeare",
  ...
  const sampleTopicData = {
    topic_name: "General Knowledge Quiz",
    topic_description: "Test your knowledge on various subjects including geography, science, literature, and more."
  };
  
  const [questionData, setQuestionData] = useState(sampleQuestionData);
  ...
  const { socket = mockSocket, isConnected = true } = useSocket ? useSocket() : { socket: mockSocket, isConnected: true };
  ...
  useEffect(() => {
    // Check if localStorage has data and if not, use sample data
    ...
    if (questionsData) {
      ...
      // Use sample data if no data in localStorage
      setQuestionData(sampleQuestionData);
    }

    // Similar approach for topic data
  ...
  const submitScore = (finalScore) => {
    ...
    // Get gameId from localStorage or use a default value
    const gameId = localStorage.getItem("gameId") || "sample-game-id";
        ...
        {timeUp ? (
          ...
          <div className="quizCard">
            ...
            <div className="progressBar">
              <div className="progressIndicator" style={{ width: `${((currentQuestionIndex + 1) / 5) * 100}%` }}>
              </div>
